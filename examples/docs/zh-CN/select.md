...
搜索确定按钮方法绑定由外部传入
:::demo 搜索框带有`确定`及`取消`按钮，必选保证`tselect`是`true`，切必须传入确定和取消的回调函数`sure-click`,`clear-click`函数，无参，尽可执行你想要执行的函数
```html
<template>
  <el-select 
    v-model="value" 
...
| tselect | 让下拉框带有底部确定和取消按钮，需要配合 `sure-click`和`clear-click`回掉函数使用 | boolean | - | false |
...
| sure-click | 点击确定时调用，需要属性`tselect`为`true`时才可生效 | — |
| clear-click | 点击清空时调用，需要属性`tselect`为`true`时才可生效 | — |
